...
?>
<form method="post" class="card" style="max-width:720px" id="surveyForm" enctype="multipart/form-data">
  <?= Csrf::field() ?>
    ...
    <div class="form-group survey-q" data-qid="<?= (int)$q['id'] ?>">
      ...
      <?php if ($q['question_type'] === 'single' && is_array($opts)): ?>
        <?php foreach ($opts as $o): ?>
          <label style="font-weight:400;display:block"><input type="radio" name="q_<?= (int)$q['id'] ?>" value="<?= View::e($o) ?>"> <?= View::e($o) ?></label>
        <?php endforeach; ?>
      ...
      <?php elseif ($q['question_type'] === 'file'): ?>
        <input type="file" name="q_<?= (int)$q['id'] ?>">
